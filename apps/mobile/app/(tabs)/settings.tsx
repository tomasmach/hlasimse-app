...
import { ArrowRight, Bell, DownloadSimple, GearSix, ShieldWarning, SignOut, Trash, UserCircle } from "phosphor-react-native";
...
function Row({ label, detail, icon: Icon, onPress, testID, danger = false }: { label: string; detail?: string; icon: React.ComponentType<any>; onPress: () => void; testID?: string; danger?: boolean }) {
  return <Pressable testID={testID} onPress={onPress} className="min-h-[72px] py-4 border-b border-sand flex-row items-center gap-4" accessibilityRole="button" accessibilityLabel={label} accessibilityHint={detail}><Icon size={23} color={danger ? COLORS.error : COLORS.charcoal.default} /><View className="flex-1"><Text className={`font-body-semibold text-base ${danger ? "text-error" : "text-charcoal"}`}>{label}</Text>{detail ? <Text className="font-body text-sm leading-5 text-muted mt-1">{detail}</Text> : null}</View><ArrowRight size={20} color={COLORS.muted} /></Pressable>;
}
